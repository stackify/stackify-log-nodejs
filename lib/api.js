var os      = require('os'),
    ...
    debug   = require('./debug'),
    URL     = require('url').URL,
    unix_socket = require(path.join(__dirname, '../lib/transport/unix_socket')),
    http_client = require(path.join(__dirname, '../lib/transport/http-client')),
    ...
    */
    ...
    sendToSocket: function sendToSocket(messages, cb) {
        try {
            ...
            async.retry({
                times: CONFIG.MAX_RETRIES, // max tries = 6
                interval: CONFIG.DELAY.SOCKET_DELAY // delay = 10 sec
            }, function (callback) { return unix_socket.send(data, cb) },
                function(err, result) {
                    if (err) {
                        var err_message = '[Stackify Log API Error] sendToSocket Sending failed:' + JSON.stringify(err, null, 2);
    ...
    /**
     *** Send log messages via http request
     *** messages - array of messages
     *** cb - callback function
     */
    sendToHttpClient: function sendToHttpClient(messages, cb) {
        try {
            ...
            }, function (callback) { return http_client.send(data, cb) },
                function(err, result) {
                    if (err) {
                        var err_message = '[Stackify Log API Error] sendToHttpClient Sending failed:' + JSON.stringify(err, null, 2);
                        debug.writeSync(err_message);
                        throw new Error(err); // Error still thrown after retrying N times, so rethrow.
                    }
              });
        } catch (error) {
            console.log('sendToHttpClient error:', error)
            debug.writeSync('\nSending failed. error stack:' + JSON.stringify(error));
        }
    }
}
