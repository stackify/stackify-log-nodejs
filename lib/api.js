    ...
    */
    ...
    transportType: function transportType(settings) {
        ...
        if (type == 'agent_socket') {
            ...
            if (!settings.appName || (settings.appName && typeof (settings.appName) !== 'string')) {
                ...
                throw new TypeError('[Stackify Log API Error] You have to pass an Application Name (Must be a string)');
            }
    ...
    sendToSocket: function sendToSocket(messages, cb) {
        ...
        try {
            ...
            }, function (callback) { return unix_socket.send(data, callback) },
                function(err, result) {
                    if (err) {
                        var err_message = '[Stackify Log API Error] Sending failed:' + JSON.stringify(err, null, 2);
                        debug.writeSync(err_message);
                        throw new Error(err); // Error still thrown after retrying N times, so rethrow.
                    } else {
                        if (result) {
                            debug.write('Successfully send ' + length + ' messages.');
                        }
                    }
              });
        } catch (error) {
