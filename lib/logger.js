    ...
    sendLogs = function (cb, shutdown) {
        ...
        try {
            ...
            if (data.length && CONFIG.APP_DETAILS) {
                ...
                if (CONFIG.TRANSPORT.toLowerCase() === 'agent_socket') {
                    api.methods.sendToSocket(data, success); // send to unix domain socket
                } else {
                    api.methods.postLogs(data, success, shutdown); // send directly to stackify-api server
                }
            }
        } catch (error) {
