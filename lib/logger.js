var util    = require('util'),
    ...
    sendLogs = function (cb) {
        ...
        if (data.length && CONFIG.APIKEY) {
            // queue has messages and authorized - set the flag, post data
            api.methods.postLogs(data, success);
            flag = true;
        } else {
            // queue is empty, set timeout for the next request, switch the flag
            timeout = setTimeout(sendLogs, delay);
            flag = false;
        }
    },
...
module.exports.methods = {
    ...
    */
    ...
    // stop all current requests, push exception to the queue and send all the data
